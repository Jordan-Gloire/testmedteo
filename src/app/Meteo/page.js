...
export default function Meteo() {
    ...
    const lang = "fr"
    const units = "metric"
    const url = `https://api.openweathermap.org/data/2.5/weather?q=${city}&lang=${lang}&units=${units}&appid=${key}`
        ...
        <>
             <div className='bg-gray-50 h-screen'>
                <div className='flex justify-between items-center'>
                    ...
                    <p className='px-10 pt-4 animate__animated animate__shakeY'> | météoCG</p>
                </div>
                
                <div className='flex justify-center items-center pt-10 animate__animated animate__flipInX'>
                    <input
                ...
                <div className='  text-center ' >
                    {
                        data && (
                            ...
                            <div className='animate__animated animate__fadeIn'>
                                ...
                                <p className='text-6xl font-bold text-black pt-8'>{data.main.temp}°</p>
                                <p className='text-2xl font-light text-black pt-2'>Température max: {data.main.temp_max}°</p>
                                <p className='text-2xl font-light text-black pt-2'>Température min: {data.main.temp_min}°</p>
                                <p className='text-2xl font-light'>Lever du soleil: {new Date(data.sys.sunrise * 1000).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                                <p className='text-2xl font-light'>Coucher du soleil {new Date(data.sys.sunset * 1000).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                                <p className='text-2xl font-light'>Vent: {data.wind.speed}  km/h</p>
                            </div>
                        )
                    }
                </div>
